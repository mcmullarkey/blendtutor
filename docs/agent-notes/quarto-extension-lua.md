...
- 2026-08-04 (#152): **quarto render auto-creates `<dir>/.gitignore`** (contents: `/.quarto/` + `**/*.quarto_ipynb`) in every directory it renders — not just demo-book (demo-standalone/ also got one before its #227 removal). It shows as untracked `?? <dir>/.gitignore` and must be rm'd before commit (root .gitignore already covers `.quarto/`).
