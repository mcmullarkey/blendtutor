...
- Repo context via `actions/checkout@v5` (not `GH_REPO` env) in the release job — checkout-before-gh-usage is repo convention (the original model, docs.yml verify-live, was removed in #233) and also gives `--generate-notes` a real commit history to diff against. gh resolves the repo from GH_REPO env or git remotes under cwd only; GITHUB_REPOSITORY is ignored.
