...
### Progress
- [x] 2026-08-10 — impl complete (branch 215-path-scrub): scrubbed 14 abs-path occurrences across 8 docs/evals/ files (2 eval.json lesson fields → repo-root-relative; 6 run.yaml lesson+runner → repo-root-relative; ../../../../ relative forms untouched); creating-lessons.md Step 9 gained scrub instruction naming worktree-issue-N/ + worktree-*/blendtutor-* classes; whole-game.md:119-121 no longer claims absolute lesson path; check-docs.sh gained AC-215 /Users/ pin (lines 103-113, fails closed); minimal root pyproject.toml added (probe parse-check needs yaml — see Surprises); evidence bundle at docs/evidence/215/ (probe.log, destructive-proof.log, parse-check.log). Verbatim probe chain exit 0.
...
- 2026-08-10 — added minimal root pyproject.toml (name blendtutor-docs-tooling, single dep pyyaml>=6.0) + committed uv.lock + gitignored /.venv/ — required for the verbatim probe's `uv run python -c "import json,yaml,..."` (bare uv env has no yaml; Rust workspace had no pyproject)

### Surprises & Discoveries
- Spec-side env blind spot: the probe's `uv run python -c "import json,yaml,..."` cannot import yaml in a bare uv environment, and this Rust workspace had no pyproject.toml at all. Resolved by adding a minimal root pyproject.toml (blendtutor-docs-tooling, dependencies=["pyyaml>=6.0"]) plus committed uv.lock and gitignored /.venv/, which makes the verbatim probe deterministic for any contributor. Same class as whole-game-tutorial AC-1's /tmp + GNU-timeout blind spots.
- Destructive-proof first FAILED because the scratch pollute file lived in a dot-prefixed dir (docs/evals/.scratch-pollute-215): rg ignores hidden directories by default, so the pin correctly saw nothing and exited 0. Real pollution is non-hidden committed eval.json/run.yaml — reran the proof with a non-hidden scratch path and the pin failed closed (exit 1) exactly as intended. Lesson: a destructive-proof must pollute where the guard actually looks, not in a shadow of it.
