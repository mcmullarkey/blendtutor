...
- [x] 2026-06-25 11:00 — Fix(review): cycle 1 — idle pill text contrast (color rule-level fix) + border color value (#444444→#808080) + 5 new contrast pairs in clause 8
- [x] 2026-06-25 11:30 — Fix(review): cycle 2 — idle pill light-mode regression (scope `surface` to `@media` block, keep `text-secondary` as default) + verdict `<strong>` status-pass/fail token values (#4caf50→#66bb6a, #f44336→#ff5252) + light-mode contrast loop
...
- 2026-06-25 (cycle 2) — Light-mode contrast loop added as parallel check. Light mode excludes `border` and `status-idle-on-surface` pairs because: (a) 1px border lines are not meaningful UI components requiring 3:1 contrast per WCAG SC 1.4.11, and (b) `status-idle` is only used as a background in light mode, never as text on surface.
- 2026-06-25 (cycle 2) — Idle pill dark-mode override placed as a separate `#lesson-status[data-status="idle"]` rule inside the `@media` block (after `:root` closes), rather than grouping with the `:root` token declarations. This keeps token declarations in `:root` and rule overrides as sibling rules, matching CSS best practice.

### Status
- `status` field in frontmatter currently: `complete` (before this fix), but the review cycle 3 identified a cascade source-order bug that needs fixing.

## Progress (review cycle 3)
- [x] 2026-06-25 12:00 — Fix(review): cycle 3 — CSS cascade source-order bug: @media block was at line 80, BEFORE the default `#lesson-status[data-status="idle"]` rule at line 272. Both have same specificity (1,1,0); @media does NOT add specificity per CSS cascading spec. Later source order wins → dark-mode idle pill override was dead code (1.36:1, not 4.70:1). Fix: moved entire @media block to end of file, after all semantic rules. The test checks token VALUES from parsed `:root`, not computed CSS styles, so it passed even with dead code. Updated alongside:
  - Updated workspace + feedback section comments referencing `@media (prefers-color-scheme: dark)` to avoid exact string (confounded test's `css.find()` which now finds them first since @media is at end of file)
  - Updated hex-scan tests (3 locations: build.rs ×2 for webR/Pyodide, site/mod.rs ×1 for unit test) to exclude the dark-mode @media block from the "no hardcoded hex" check
  - Warn comment now avoids `#lesson-status[data-status="idle"]` and `:root` strings that confound test regex matching
...
- PR review finding (review cycle 2): The cycle-1 idle pill fix (`color: var(--bt-color-surface)` unconditional) regressed light mode: `surface (#ffffff) on status-idle (#e8e8e8)` = 1.23:1 in light mode. Fixed by scoping the `surface` override to the `@media (prefers-color-scheme: dark)` block and keeping `text-secondary` as the default. NO single token works unconditionally for the idle pill — `surface` fails light (1.23), `text-secondary` fails dark (1.36), `text-primary` fails dark (2.69). The only correct approach is `@media` scoping.
- PR review finding (review cycle 2): Verdict `<strong>` elements use `status-pass`/`status-fail` on `success-bg`/`danger-bg` backgrounds in the BYOK feedback panel. The original dark values (`#4caf50` on `#1b3d1b` = 4.37:1, `#f44336` on `#3d1b1b` = 4.16:1) failed 4.5:1 for normal text. The fix lightened both tokens to `#66bb6a` (5.13:1) and `#ff5252` (4.80:1). These tokens are NOT listed in clause 5's directional sanity check (they're accent colors, not surface/bg or text), so the directional check still passes despite them lightening instead of darkening.
- Adding comments containing hex values (e.g., `/* Idle pill text on #888888 */`) triggers clause 7's hardcoded-hex scan — all hex literals must be inside a `:root` block. Comments with hex values must avoid the `#` prefix or omit the value entirely.
- Three test files assert the count of `#lesson-status[data-status="..."]` rules: `crates/cli/tests/build.rs` (2 occurrences for webR+Pyodide) and `crates/core/src/site/mod.rs` (1 occurrence for unit test). Adding the dark-mode idle override increased the count from 4 to 5, needing updates in all 3 locations.
- CSS cascade source-order bug (review cycle 3): When a `@media (prefers-color-scheme: dark)` block is placed BEFORE a default rule with the same specificity, the override is dead code — `@media` does NOT add specificity per CSS Cascading spec, and later source order wins. The tests check token VALUES from parsed `:root` declarations (via brace-counted HashMap), NOT computed CSS styles, so all tests passed even with dead code. This is a fundamental testing-level mismatch: the test verifies "are the dark-mode token values correct?" but NOT "does the dark-mode override actually apply in the cascade?" The fix is to move the entire `@media` block to the END of the file (after all semantic rules), ensuring the @media-scoped rules win by source order. Moving the block required updating 3 more test locations that scan hardcoded-hex — the @media block's `:root` hex values (`#1e1e1e`, `#66bb6a`, etc.) triggered the "no hex outside :root" checks since they were now in the `after_marker` / `byok_section` ranges.
- Moving the @media block to the end of the file confounded 3 test functions that use `css.find("@media (prefers-color-scheme: dark)")` to locate the @media block. Previously, the @media block was right after `:root`, so the FIRST occurrence of the string in `after_root` (text after light `:root`) was the actual @media rule. After moving it to the end, comments at lines 132/290 (workspace/feedback sections) which say "Dark mode is handled by @media (prefers-color-scheme: dark)" were found FIRST, causing `css_decl_block` to find the next `{` in the semantic rule section (e.g., `.lesson-picker {`) and return a wrong block body. Fix: removed the exact string `@media (prefers-color-scheme: dark)` from those comments, replacing with "the dark-mode @media block" instead.
